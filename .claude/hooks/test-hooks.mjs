...
const CASES = [
  ...
  // Tùy chọn toàn cục của git đứng trước lệnh con, và cờ -f không đứng ngay sau clean (từng lọt).
  [bash('git -C . push origin main'), 2],
  [bash('git -c user.name=x push origin main'), 2],
  [bash('git --no-pager push'), 2],
  [bash('git -C . reset --hard HEAD~1'), 2],
  [bash('git reset HEAD~1 --hard'), 2],
  [bash('git clean -d -f'), 2],
  [bash('git clean --force'), 2],
  [{ tool_name: 'PowerShell', tool_input: { command: 'git -C . push origin main' } }, 2],
  [{ tool_name: 'PowerShell', tool_input: { command: 'Get-ChildItem apps -Recurse | Remove-Item -Force' } }, 2],
  [bash('find apps -name "*.ts" -delete'), 2],
  [bash('find apps -exec rm {} +'), 2],
  [bash('docker volume rm testctylongdo_mongo-data'), 2],
  // Các cách đọc .env khác cat (từng lọt).
  [bash('base64 apps/api/.env'), 2],
  [bash('xxd apps/api/.env'), 2],
  [bash(`node -e "console.log(require('fs').readFileSync('apps/api/.env','utf8'))"`), 2],
  [{ tool_name: 'PowerShell', tool_input: { command: 'Select-String -Path apps/api/.env -Pattern SECRET' } }, 2],
  // Chữ trong commit message và mẫu grep đọc từ pipe không phải là lệnh (từng bị chặn nhầm).
  [bash('git commit -m "docs: nhắc không dùng git push"'), 0],
  [bash('git commit -m "fix: thêm cờ -n cho lệnh"'), 0],
  [bash('git commit -m "feat: hỗ trợ -nhanh"'), 0],
  [bash('git commit -m "docs: cat .env không được"'), 0],
  [bash(`git commit -m "$(cat <<'EOF'\nfix(claude): chặn git -C . push và git clean -d -f\n\nKhông chặn nhầm chữ -n trong message.\nEOF\n)"`), 0],
  [bash('git ls-files | grep -i .env'), 0],
  // Nhưng message không che được lệnh thật đi sau.
  [bash('git commit -m "x" && git push'), 2],
  [bash('git commit -m "x" -n'), 2],
  [bash('git ls-files | grep SECRET apps/api/.env'), 2],
