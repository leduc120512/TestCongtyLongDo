...
## Luật cần soát
- **Phân tầng API**: `routes/*` chỉ `kiemTra(schema)` + gọi service + bọc `{ data }`. `services/*` chứa nghiệp vụ và quyền, không import `mongodb`. `repositories/*` là nơi duy nhất truy vấn Mongo.
- **Phân tầng web**: `pages`/`components` → `hooks` → `api`. Component không gọi `fetch`/`goiApi`.
...
- **Response**: `{ data }`, `{ data, meta: { page, limit, total } }`, lỗi `{ error: { code, message } }` với `code` thuộc `MA_LOI`. Id trả ra là chuỗi, không lộ `_id`, `tuKhoa`, `hanSapXep`, `deletedAt`.
- **Mongo**: không ghi `undefined`/`null` (dùng `boUndefined`, `null` → `$unset`); tham chiếu lưu `ObjectId`; truy vấn danh sách mới có index tương ứng.
- **Giao dịch**: ghi công việc và ghi lịch sử (và tăng bộ đếm mã) phải nằm trong cùng `kho.giaoDich` — trong service thường qua `ghiCoKhoa`, có điều kiện `trangThai` + `phienBan`.
- **Nghiệp vụ**: chuyển trạng thái đi qua `xetChuyenTrangThai`; quyền đi qua `tinhQuyen`/`xacDinhVaiTro`; người ngoài nhận 404; xóa là xóa mềm; `HOAN_THANH` không sửa/xóa.
- **Thời gian**: `batDau`/`hetHan` là chuỗi `YYYY-MM-DD`; "hôm nay" dùng `homNayVN()`, không dùng `new Date().toISOString().slice(0,10)` hay giờ máy.
