...
- Đặt tên: code tiếng Anh (file kebab-case, component PascalCase). Hợp đồng dữ liệu giữ tiếng Việt không dấu theo mẫu của đề: tên trường JSON/Mongo (`nguoiGiaoId`, `trangThai`...), giá trị enum, URL API, mã lỗi. Chữ hiển thị, thông báo lỗi, comment: tiếng Việt.
...
## Phân tầng (bắt buộc)
- API: `routes/` chỉ `validate(Schema, req.body)` → gọi service → bọc `{ data }`. `services/` chứa nghiệp vụ + quyền, **không import `mongodb`**, làm việc qua `repositories/interfaces.ts`. `repositories/` là nơi duy nhất truy vấn Mongo.
...
- Mongo: không ghi `undefined`/`null` (dùng `stripUndefined`; trong thay đổi `null` = `$unset`). Tham chiếu lưu `ObjectId`, đổi sang chuỗi ở repository.
...
- Mọi lệnh ghi công việc đi qua `writeWithLock`: điều kiện `trangThai` + `phienBan` (khóa lạc quan, người sau nhận `XUNG_DOT`) và ghi lịch sử trong cùng `store.transaction`. Không ghi công việc rồi ghi lịch sử ở hai bước rời.
